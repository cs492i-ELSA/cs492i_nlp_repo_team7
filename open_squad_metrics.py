...
# We unified prediction functions for validation/inference to this fuction.
def select_best_predictions(all_nbest_json, null_ths=0.4, is_test=True):
    best_answer_max_prob = collections.OrderedDict(collections.OrderedDict())
    ...
    for qas_id, nbest_json in all_nbest_json.items():
        if is_test:
            ...
            qa_id_without_s = qas_id
        text = nbest_json[0]["text"]
        prob = nbest_json[0]["probability"]

        if qa_id_without_s not in best_answer_max_prob:
            best_answer_max_prob[qa_id_without_s] = collections.OrderedDict({text: nbest_json[0]["probability"]})
        else:
            if text in best_answer_max_prob[qa_id_without_s]:
                if best_answer_max_prob[qa_id_without_s][text] < nbest_json[0]["probability"]:
                    best_answer_max_prob[qa_id_without_s][text] = nbest_json[0]["probability"]
            else:
                best_answer_max_prob[qa_id_without_s][text] = nbest_json[0]["probability"] 
...
def compute_predictions_logits(
    ...
    if not is_test:
        ...
        return select_best_predictions(all_nbest_json, is_test=False)
    else:
