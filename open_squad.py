...
class SquadProcessor(DataProcessor):
    ...
    def get_pretrain_examples(self, data_dir, filename=None, qa_style=False):
        if data_dir is None:
            data_dir = ""
        ...
        return self._create_pretrain_examples(input_data, qa_style=qa_style)       
    ...
    # Create examples for pretraining in either span/qa style
    def _create_pretrain_examples(self, input_data, qa_style=False):
            print("start loading pretraining examples")
            examples = []
            showcases = 20
            ...
            for entry in tqdm(input_data[:]):
                ...
                # for end-to-end matching of question/context paragraph
                first_para = None
                first_pi = None
                qa_para = None
                for pi, paragraph in enumerate(reversed(entry["paragraphs"])):
                    ...
                    # In qa style, original question is used as question
                    if qa_style:
                        qa_para = question_text

                    # store first paragraph to be use as context paragraph for last question
                    if qa_para is None:
                        first_pi, first_para = -1, paragraph
                        for sentence in context_text.split("."):
                            if answer_text in sentence:
                                qa_para = "[MASK]".join(sentence.split(answer_text, 1))
                                break
                        continue

                    qas_id = "{}[SEP]{}[SEP]{}".format(qa_para, answer_text, pi)
                    ...
                    # In span style, current paragraph is processed (answer span is masked) and become question of next paragraph
                    if not qa_style:
                        for sentence in context_text.split("."):
                            if answer_text in sentence:
                                qa_para = "[MASK]".join(sentence.split(answer_text, 1))
                    ...
                    if per_qa_paragraph_cnt > 3:
                        break
                    if showcases > 0:
                        print("[Examples] qid: {}, q: {}, c:{}, a: {}".format(example.qas_id, example.question_text, example.context_text, example.answer_text))
                        showcases -= 1
                    examples.append(example)

                # match last question with first paragraph
                for i in range(1):
                    ...
                    if qa_para is None:
                        break
                    pi, paragraph = first_pi, first_para
                    title = paragraph["title"]
                    context_text = str(paragraph["contents"])
                    if context_text is None:
                        continue
            ...
            print("[{}] Has Answer({}) / No Answer({})".format("pretrain", has_answer_cnt, no_answer_cnt))
            return examples
...
class SquadResult(object):
    ...
    def __init__(self, unique_id, start_logits, end_logits, start_top_index=None, end_top_index=None, cls_logits=None, ans_prob=None):
        self.start_logits = start_logits
        self.end_logits = end_logits
        self.unique_id = unique_id
        self.ans_prob = ans_prob
        if start_top_index:
