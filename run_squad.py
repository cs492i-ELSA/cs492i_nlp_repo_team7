...
# Variant of Electra for Question and Answering Downstream Task,
# by exploiting addtional binarcy classification layer for starrt/end of answer span.
# We added external verifier 'ifv' for prediction of answerability
...
class ElectraForQuestionAnswering(ElectraPreTrainedModel):
    ...
    Returns:
        ...
        cls_logits = self.ifv(sequence_output[:, 0])
        ans_probs = self.softmax(cls_logits)[:, 1]
        outputs = (start_logits, end_logits, ans_probs) + outputs[1:]
        ...
        if start_positions is not None and end_positions is not None:
            ...
            answerability = (start_positions != 0) + (end_positions!=0)
            answerability = answerability.type(torch.LongTensor).cuda()

            loss_fct = CrossEntropyLoss(ignore_index=ignored_index)
            start_loss = loss_fct(start_logits, start_positions)
            end_loss = loss_fct(end_logits, end_positions)
            cls_loss = loss_fct(cls_logits, answerability)
            total_loss = (start_loss + end_loss + cls_loss) / 2
...
def predict(args, model, tokenizer, prefix="", val_or_test="val"):
    ...
    dataset, examples, features = load_and_cache_examples(
        model, args, tokenizer, evaluate=True, output_examples=True,
        val_or_test=val_or_test,
    )
    t = time.time() - t
    logger.info("loading dev data takes {:.3f} seconds".format(t))
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            if len(output) >= 5:
                ...
                start_logits, end_logits, ans_prob = output
                result = SquadResult(unique_id, start_logits, end_logits, ans_prob=ans_prob)
...
# Load examples and makde datasets to be used in training/verification, and we made two changes.
# 1. We improved it to use caches (session where examples are cached is specified through arguments)
# 2. We added optioni 'mix', in order to reconstruct question/answer pair
